...
(function (root, factory) {
  ...
  function Popper (reference, popper, options) {
    ...
    let isNotDefined = typeof popper === "undefined" || popper === null; // 判断是否定义了 popper
    let isConfig =
  ...
  Popper.prototype.parse = function (config) {
    ...
    if (config.arrowTagName) {
      // 如果有箭头的标签名
      let arrow = d.createElement(config.arrowTagName); // 创建相应标签
  ...
  Popper.prototype._getPosition = function (popper, reference) {
    let container = getOffsetParent(reference); // 获取父元素的偏移
  ...
  Popper.prototype._setupEventListeners = function () {
    ...
    if (this._options.boundariesElement !== "window") {
      let target = getScrollParent(this._reference); // 获取相关元素可滚动的父级
  ...
  Popper.prototype._getBoundaries = function (data, padding, boundariesElement) {
    ...
    if (boundariesElement === "window") {
      ...
      // 如果边界元素时视窗
      let offsetParent = getOffsetParent(this._popper); // 寻找 popper 定位的父元素
      let scrollParent = getScrollParent(this._popper); // 寻找 popper 可滚动的父元素
      let offsetParentRect = getOffsetRect(offsetParent); // 寻找 offsetParent 定位的父元素

      // 如果 popper 是固定定位，就不需要减去边界的滚动值
      let scrollTop =
        data.offsets.popper.position === "fixed" ? 0 : scrollParent.scrollTop;
      let scrollLeft =
        data.offsets.popper.position === "fixed" ? 0 : scrollParent.scrollLeft;
  ...
  Popper.prototype.runModifiers = function (data, modifiers, ends) {
    let modifiersToRun = modifiers.slice(); // 创建一个新的修饰符数组
    if (ends !== undefined) {
      // 如果制定了 ends，就截断该数组
  ...
  Popper.prototype.isModifierRequired = function (requesting, requested) {
    let index = getArrayKeyIndex(this._options.modifiers, requesting); // 获取要判断的修饰符的索引
  ...
  Popper.prototype.modifiers.shift = function (data) {
    ...
    let basePlacement = placement.split("-")[0]; // 基本位置
    let shiftVariation = placement.split("-")[1]; // 偏移位置
  ...
  Popper.prototype.modifiers.preventOverflow = function (data) {
    let order = this._options.preventOverflowOrder; // 检测顺序
    let popper = getPopperClientRect(data.offsets.popper);
  ...
  Popper.prototype.modifiers.arrow = function (data) {
    ...
    let popper = getPopperClientRect(data.offsets.popper);
    let reference = data.offsets.reference;
    let isVertical = ["left", "right"].indexOf(placement) !== -1; // 是否垂直
  ...
  function getOuterSizes (element) {
    ...
    // original method
    // 原始方法
    let styles = root.getComputedStyle(element); // 获取计算后的样式
    let x = parseFloat(styles.marginTop) + parseFloat(styles.marginBottom); // 上下边距
    let y = parseFloat(styles.marginLeft) + parseFloat(styles.marginRight); // 左右边距
    let result = {
